...
export default function CompanyDataPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/dashboard/recent-vouchers?limit=20&companyId=${activeCompany.id}`),
      fetch(`/api/dashboard/stock-summary?companyId=${activeCompany.id}`),
    ])
      .then(([tb, rv, ss]) => Promise.all([tb.json(), rv.json(), ss.json()]))
      .then(([tbData, rvData, ssData]) => {
        setLedgers(tbData)
        setVouchers(rvData)
        setStockItems(ssData)
      })
      .catch(() => {})
